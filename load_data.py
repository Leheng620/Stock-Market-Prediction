...
class TransformerAdditionalFeaturesDataset(Dataset):
    def __init__(self, tickers, look_back=30, train=True):
        sequences = []  # (N, L, D)
        targets = []  # (N, D)
        scalers = []
        for ticker in tickers:
            df = pd.read_csv(f'./arima/{ticker}_arima.csv', index_col='date', parse_dates=True)
            # df.drop(['Unnamed: 0', 'arima_res'], axis=1, inplace=True)
            df = df[['adj.close', "open", 'high', 'low', 'volume', "macd", "rsi_30",
                "cci_30", "dx_30", "close_30_sma", "close_60_sma", "arima_res"]]
...
class TransformerMultiFeaturesFilteredDataset(Dataset):
    def __init__(self, tickers, look_back=30, train=True):
        sequences = []  # (N, L, D)
        targets = []  # (N, D)
        scalers = []
        for ticker in tickers:
            df = pd.read_csv(f'./price_filtered/{ticker}.csv', index_col='Date', parse_dates=True)
            df = df[['Adj Close', 'High', 'Low', 'Volume']]
...
# train = get_data_loader(TransformerAdditionalFeaturesDataset, ['MSFT'], look_back=60, batch_size=32, shuffle=False)
# print(len(train))
# for it in train:
#     x, y = it
