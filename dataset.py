...
class StockDataset(Dataset):
    def __init__(self, data, tokenizer, time_window):
        self.data = data
    ...
    def __getitem__(self, idx):
        tweets = [row["text"] for row in self.data[idx:idx + self.time_window]]
        prices = [row["adj_close"] for row in self.data[idx:idx + self.time_window]]
        volumes = [row["volume"] for row in self.data[idx:idx + self.time_window]]

        # Tokenize tweets
        inputs = self.tokenizer(tweets, return_tensors="pt", padding=True, truncation=True)
        inputs['prices'] = torch.tensor(prices, dtype=torch.float32)
        inputs['volumes'] = torch.tensor(volumes, dtype=torch.float32)

        # Calculate label
        label = 1 if self.data[idx + self.time_window]["adj_close"] > self.data[idx + self.time_window - 1]["adj_close"] else 0
...
def get_data_loader(dataset, tickers, look_back=30, batch_size=32, shuffle=True, train=True):
    data = StockDataset(tickers, priceOrtweet, look_back, train)
